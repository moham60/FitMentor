...
export default function MessageNotificationsBell() {
  ...
  const headerLabel = useMemo(() => {
    if (unreadCount === 0) return 'no new message';
    if (unreadCount === 1) return ' new message';
    if (unreadCount === 2) return ' new 2 messages';
    if (unreadCount <= 10) return `${unreadCount}  new messages`;
    return `${unreadCount}  new message`;
  }, [unreadCount]);
      ...
      <DropdownMenuContent align="end" className="w-[340px] p-0">
        <div className="px-3 py-3">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            open message click on notification
          </p>
        </div>
        <DropdownMenuSeparator />
        ...
        {items.length === 0 ? (
          <div className="px-3 py-8 text-center">
            <p className="text-sm text-muted-foreground">there is no unread message</p>
            <Button
              variant="outline"
