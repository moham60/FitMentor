"""
FitMentor Chatbot — Professional PPTX Presentation Generator (ENGLISH)
"""
...
domains = [
    ("Profile & Body", GREEN, ["profiles — User info, goal, TDEE, BMR", "body_measurements — InBody scans", "inbody_results — Scan history", "weight_logs — Weight tracking"]),
    ("Workouts", BLUE, ["workout_plans — Training templates", "workouts — Completed sessions", "exercise_library — Exercise DB", "user_exercise_sets — Reps/sets"]),
    ("Nutrition", PINK, ["user_meal_plans — Meal schedules", "meals — Logged meals with macros", "food_items — Food nutrition DB", "user_meal_checkins — Compliance"]),
...
techs = [
    ...
    ("Supabase", "PostgreSQL + REST + RLS policies", "23 tables · Row Level Security"),
...
for i, (nm, desc, tags) in enumerate(techs):
    col, row = i%3, i//3
    ...
    txt(sl, x+Inches(0.15), y+Inches(0.1), Inches(3.5), Inches(0.35), nm, 16, True, WHITE)
...
routes = [("Supabase Only", "23 tables", GREEN), ("Vector Only", "FAISS DB", BLUE), ("Hybrid", "Both", PINK)]
for i, (t, d, c) in enumerate(routes):
    x = Inches(1+i*4.1)
...
for i, (ti, de) in enumerate(features):
    col, row = i%4, i//4
    card(sl, Inches(0.4+col*3.15), Inches(1.4+row*2.7), Inches(2.95), Inches(2.4), "✨", ti, de)
...
# ── Save ───────────────────────────────────────────────────────────────────────
out = "FitMentor_Chatbot_Presentation_EN.pptx"
prs.save(out)
