...
export type AnatomyMapProps = {
  /** id of selected muscle group (must match <g className="bodymap" id="..."> in the SVG) */
  selected?: string;
  /** called when user clicks a muscle group */
  onMuscleClick?: (id: string) => void;
  /** if true, dims all muscles except the selected one (requires selected) */
  dimmed?: boolean;
  /** pass a ref to read/query the SVG (optional) */
  svgRef?: React.RefObject<SVGSVGElement | null>;
};

/**
 * Male anatomy (FRONT) - extracted from ExercisesPage.tsx.
 * - Click any <g className="bodymap" id="..."> to select.
 * - Selected group gets "selected" class.
 * - If dimmed && selected => svg gets "dim" class.
 */
export function MaleFrontAnatomy({
  selected,
  onMuscleClick,
  dimmed,
  svgRef,
  ...
  useEffect(() => {
    ...
    if (dimmed && selected) s.classList.add("dim");
    else s.classList.remove("dim");
  }, [selected, dimmed, svgRef]);

  // click muscle in SVG
  useEffect(() => {
    const s = svgRef?.current;
    if (!s || !onMuscleClick) return;
...
/**
 * Male anatomy (BACK) - delegates to existing BackMuscleMap component.
 * It supports the same props contract used in ExercisesPage.tsx.
 */
export function MaleBackAnatomy(props: AnatomyMapProps) {
  return (
    <BackMuscleMap
      selected={props.selected}
      onMuscleClick={props.onMuscleClick}
      dimmed={props.dimmed}
      svgRef={props.svgRef}
    />
  );
