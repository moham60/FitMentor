...
// السطر السحري: تحويل Supabase لنسخة غير مقيدة بأسماء جداول محددة
...
export default function AIAssistantView() {
  ...
  const fetchMessages = useCallback(async (conversationId: string | null) => {
    ...
    try {
      ...
      const msgs = (data as StoredChatMessage[]) || [];
      setMessages(
        msgs.map((msg) => ({
          id: msg.id,
          role: msg.role,
  ...
  const ensureConversation = async (firstMessage: string) => {
    ...
    const conv = await createConversation(buildConversationTitle(firstMessage), false);
    return conv?.id ?? null;
  };
  ...
  const updateMessage = (id: string | null, updates: Partial<ChatItem> | ((msg: ChatItem) => Partial<ChatItem>)) => {
    setMessages((current) =>
      current.map((msg) => {
  ...
  const sendMessage = async (query: string) => {
    ...
    try {
      ...
      if (assistantMessageId) {
        setMessages((current) =>
          current.filter((msg) => !(msg.id === assistantMessageId && msg.role === 'assistant' && !msg.content))
        );
      }
      setSending(false);
    }
