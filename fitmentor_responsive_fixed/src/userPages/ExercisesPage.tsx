...
import { Check, Eraser, Focus, RotateCcw } from "lucide-react";
...
// بيانات تمارين تجريبية (بدّلها ببياناتك لاحقًا)
const EXERCISES: Exercise[] = [
  { muscle: "abdominals", title: "Crunch", equipment: "bodyweight", difficulty: "beginner" },
  { muscle: "abdominals", title: "Hanging Knee Raise", equipment: "bodyweight", difficulty: "intermediate" },

  { muscle: "obliques", title: "Side Plank", equipment: "bodyweight", difficulty: "beginner" },
  { muscle: "obliques", title: "Cable Woodchop", equipment: "machine", difficulty: "intermediate" },

  { muscle: "chest", title: "Push-up", equipment: "bodyweight", difficulty: "beginner" },
  { muscle: "chest", title: "Bench Press", equipment: "barbell", difficulty: "intermediate" },

  { muscle: "biceps", title: "Dumbbell Curl", equipment: "dumbbell", difficulty: "beginner" },

  { muscle: "forearms", title: "Wrist Curl", equipment: "dumbbell", difficulty: "beginner" },

  { muscle: "front-shoulders", title: "Overhead Press", equipment: "barbell", difficulty: "intermediate" },

  { muscle: "traps", title: "Shrugs", equipment: "dumbbell", difficulty: "beginner" },

  { muscle: "quads", title: "Squat", equipment: "barbell", difficulty: "intermediate" },
  { muscle: "quads", title: "Leg Extension", equipment: "machine", difficulty: "beginner" },

  { muscle: "calves", title: "Calf Raises", equipment: "bodyweight", difficulty: "beginner" },
];
...
export default function ExercisesPage() {
  ...
  // --- UI toggles like screenshot ---
  const [isMale, setIsMale] = useState(true);

const [equipmentOpen, setEquipmentOpen] = useState(true);

// نفس شكل الصورة: قائمة equipment كبيرة (حتى لو مش كلها مستخدمة في فلتر التمارين دلوقتي)
const EQUIPMENT_UI = [
  ...
  { key: "band", label: "Band" },
  { key: "trx", label: "TRX" },
  { key: "bosu", label: "Bosu Ball" },
  { key: "cardio", label: "Cardio" },
  { key: "recovery", label: "Recovery" },
] as const;

type EquipmentUIKey = (typeof EQUIPMENT_UI)[number]["key"];
const [equipmentUISelected, setEquipmentUISelected] = useState<Set<EquipmentUIKey>>(
  () => new Set(["featured"])
);

const toggleEquipmentUI = (k: EquipmentUIKey) => {
  setEquipmentUISelected((prev) => {
  ...
  // هنجمع العضلات من الـ SVG تلقائيا (نستهدف الـ SVG المعروض حاليا)
  const muscleIds = useMemo(() => {
    const s = isMale ? (viewMode === "front" ? svgRef.current : backSvgRef.current) : svgRef.current;
    if (!s) return [];
  ...
  }, [selected, equipment, difficulty]);

  // تحديث الـ styles على الـ SVG حسب selected/dimMode
  useEffect(() => {
    const s = isMale ? (viewMode === "front" ? svgRef.current : backSvgRef.current) : svgRef.current;
    if (!s) return;
  ...
  // click على أي عضلة داخل الـ SVG
  useEffect(() => {
    const s = isMale ? (viewMode === "front" ? svgRef.current : backSvgRef.current) : svgRef.current;
    if (!s) return;
  ...
  const titleText = selected ? (NAMES_AR[selected] ? `${selected} (${NAMES_AR[selected]})` : selected) : "اختار عضلة";
  const subtitleText = selected
    ? "استخدم الفلاتر لعرض التمارين"
    : "اضغط على عضلة من الخريطة أو اختارها من القائمة";

  return (
    <MainLayout title="Exercises" subtitle="Muscle Map & Exercise Library">
      {/* نفس روح التصميم بتاع مشروعك */}
...
<Card className="lg:col-span-4 lg:sticky lg:top-20 h-fit overflow-hidden">
  ...
  <CardContent className="p-0">
      ...
      {equipmentOpen && (
        <div className="px-4 pb-4">
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            {EQUIPMENT_UI.map((it) => {
              const checked = equipmentUISelected.has(it.key);
              return (
                <label
                  key={it.key}
                ...
                >
                  ...
                  />
                  {/* icon placeholder (لو عايز نفس أيقونات الصورة 1:1 هقولك تعملها SVGs) */}
    ...
    {/* باقي الـ filters بتاعتك (لو عايز تسيبها تحت) */}
    <div className="px-4 pb-4">
      <div className="mt-3 space-y-3">
        <div>
          <Label className="text-sm">بحث عضلات</Label>
          <Input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="مثال: abdominals"
            className="mt-1"
          />
        </div>
        ...
        <Card className="lg:col-span-8 overflow-hidden">
          ...
          <CardContent>
            {isMale ? (
              viewMode === "front" ? (
                <>
                  {/* Styles خاصة بالـ SVG فقط */}
                  <style>{`
                    /* نفس فكرة HTML بتاعتك ولكن جوّا المشروع */
                    .muscleSvg g.bodymap{
              ...
              <svg
                ref={svgRef}
                className={cn("muscleSvg w-full max-w-[560px] mx-auto h-auto w-full mc:h-[700px] 2xl:h-[90vh] 3xl:h-[95vh] 4xl:min-h-[100dvh] sm:mx-auto object-fit")}
