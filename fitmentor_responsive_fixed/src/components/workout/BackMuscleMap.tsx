...
export function BackMuscleMap({
  ...
  return (
    <div className={cn("w-full rounded-xl border bg-muted/10 p-3 overflow-auto", className)}>
      <style>{`
        .backMuscleSvg g.bodymap{
          color: hsl(var(--muted-foreground));
          cursor: pointer;
          transition: color .12s ease, filter .12s ease, opacity .12s ease;
        }
        .backMuscleSvg g#body{ pointer-events: none; }
        .backMuscleSvg g.hidden{ pointer-events: none; display:none !important; }

        .backMuscleSvg g.bodymap:hover{
          color: hsl(var(--foreground));
          filter: drop-shadow(0 10px 16px rgba(110,231,255,.12));
        }
        .backMuscleSvg g.bodymap.selected{
          color: hsl(var(--primary));
          filter: drop-shadow(0 14px 22px rgba(0,0,0,.20));
        }
        .backMuscleSvg.dim g.bodymap{ opacity: .28; }
        .backMuscleSvg.dim g.bodymap.selected{ opacity: 1; }
      `}</style>

      <svg
        ref={svgRef}
        className={cn(
          "backMuscleSvg w-full max-w-[560px] mx-auto h-auto",
          "mc:h-[700px] 2xl:h-[90vh] 3xl:h-[95vh] 4xl:min-h-[100dvh] sm:mx-auto object-fit",
          dimmed && "dim"
        )}
        ...
        onClick={(e) => {
          const g = (e.target as SVGElement).closest("g.bodymap") as SVGGElement | null;
          if (g?.id && g.id !== "body" && onMuscleClick) {
            onMuscleClick(g.id);
          }
        }}
      >
        ...
        <g
          id="traps"
          className={cn("bodymap text-mw-gray active:text-mw-red-700 lg:hover:text-mw-red-100", {
            selected: selected === "traps",
          })}
        >
          <path
        ...
        <g
          id="traps-middle"
          className={cn("bodymap text-mw-gray active:text-mw-red-700 lg:hover:text-mw-red-100", {
            selected: selected === "traps-middle",
          })}
        >
          <path
        ...
        <g
          id="rear-shoulders"
          className={cn("bodymap text-mw-gray active:text-mw-red-700 lg:hover:text-mw-red-100", {
            selected: selected === "rear-shoulders",
          })}
        >
          <path
        ...
        {/* Hidden joints (can be toggled on if needed) */}
        {isJoints && (
          <>
